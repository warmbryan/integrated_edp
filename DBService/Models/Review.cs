...
namespace DBService.Models
{
    public class Review
    {
        ...
        {
            string SQL = "SELECT r.*,b.name from Review as r INNER JOIN branch as b ON r.branchId = b.id where customerId = @paraId ORDER BY r.dateCreated DESC";
            using (SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["MyDBConnection"].ToString()))
            {
                using (SqlDataAdapter sda = new SqlDataAdapter(SQL, conn))
                {
        ...
        {
            string SQL = "SELECT r.*,b.name from Review r INNER JOIN Branch b on r.branchId = b.id where numReport > 0 order by numReport DESC";
            using (SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["MyDBConnection"].ToString()))
            {
                using (SqlDataAdapter sda = new SqlDataAdapter(SQL, conn))
                {
